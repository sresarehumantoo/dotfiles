// Vendored from sahaj-b/ghostty-cursor-shaders (MIT), with a local delta.
// https://github.com/sahaj-b/ghostty-cursor-shaders
// Modifications are marked `LOCAL DELTA` (two places) — re-apply on re-vendor.
//
...
// LOCAL DELTA vs upstream sahaj-b/ghostty-cursor-shaders (see also mainImage).
// Upstream measures this in cursor *heights*, which penalises horizontal moves
// by the font's cell aspect ratio (~0.52 for Hack), so h/l needed 3 cells to
// trail where j/k needed 2 — in tmux copy-mode, left/right never fired. We
// measure in cells per axis instead, and 0.9 puts a single-cell move over the
// line. Cost: typing is also single-cell-right movement, so every typed
// character trails too. Raise above 1.0 to go back to held-key-only trails.
const float THRESHOLD_MIN_DISTANCE = 0.9; // min distance to show trail (units of CELLS, any axis)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    // LOCAL DELTA: distance in cells, not cursor heights, so the gate is
    // isotropic in grid terms (upstream: distance(centerCC, centerCP) vs
    // currentCursor.w * THRESHOLD). .zw is the cursor's (width, height).
    float lineLength = length((centerCC - centerCP) / max(currentCursor.zw, vec2(1e-6)));
    float minDist = THRESHOLD_MIN_DISTANCE;
